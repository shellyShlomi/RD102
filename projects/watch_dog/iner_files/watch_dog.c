...
int WDStart(char **argv, int check_ratio, int beats_interval)
{
    ...
    watchdog_elem->is_WD = !strcmp(argv[0], "WD_file_for_user");
...
static int ViabilityTask(void *param)
{
    ...
    Reset();
    DEBUG printf("\nViabilityTask, to_stop: %d ", to_stop);
    DEBUG printf("is_WD: %d counter: %lu \n", ((watchdog_t *)param)->is_WD, counter);
    ...
    if ((0 == atomic_load(&counter)))
    {
        if (((watchdog_t *)param)->is_WD) /*im WD*/
        {
            ...
            execv(((watchdog_t *)param)->argv[0], ((watchdog_t *)param)->argv);
        }
        else
        {
            Reset();
            DEBUG printf("WD is dead\n\n");
            ...
            if (0 == pid_child_T2)
            {
                ...
                unsetenv(WD_PID);
                execv("WD_file_for_user", ((watchdog_t *)param)->argv);
            }
