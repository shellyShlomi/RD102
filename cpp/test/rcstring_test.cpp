...
static void TestOperators()
{
    ...
    std::cin >> is_str; //expect numbers in the following format "(4,7)"
...
static void TestOperatorIdx()
{
    ...
    const RCString str(static_cast<const RCString>(str_arr[idx]));
    ...
    ValidStrEqual(str.CStr(), "shelly", "TestOperatorIdx failed at line: ", __LINE__);
...
static void TestRefCounter()
{
    ...
    for (size_t i = 0; i < SIZE; ++i)
    {
        Validate(str_arr[i] != str1, "RefCounter failed at line: ", __LINE__);
    }

    RCString str2("hello");

    Validate(str2 != str1, "RefCounter failed at line: ", __LINE__);

    // str2 = str2;
    // Valid(str2 == str2, "RefCounter failed at line: ", __LINE__);
