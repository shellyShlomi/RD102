...
#define MAX_BIT sizeof(bit_arr_t) * CHAR_BIT
#define SIZE_LUT 256 /* size char + 1 */
...
{
    shifts = shifts & (MAX_BIT - 1);

    return ((bit_arr >> shifts) | (bit_arr << (MAX_BIT - shifts)));
}
...
{
    shifts = shifts & (MAX_BIT - 1); /* modul by 63 */
...
bit_arr_t BitArrMirrorLUT(bit_arr_t bit_arr)
{
    ...
    static unsigned char mirror_lut[SIZE_LUT] =
        {
    ...
    {
        mirror = ((bit_arr_t)mirror_lut[(unsigned char)((SIZE_LUT - 1) & bit_arr)]);
        reverse |= mirror << CHAR_BIT * i;
        bit_arr = bit_arr >> CHAR_BIT;
    }
...
char *BitArrToString(bit_arr_t bit_arr, char *dest)
{
    ...
    dest += MAX_BIT; /* move right 64 bits */
    *dest = '\0';

    /* fill dest from right to left by 0 or 1  */
    for (i = 0; i < MAX_BIT; ++i)
    {
        --dest;
        *dest = (bit_arr & 1) + '0';
...
size_t BitArrCountOnLUT(bit_arr_t bit_arr)
{
    ...
    static unsigned char count_lut[SIZE_LUT] =
        {
    ...
    {
        count += (size_t)count_lut[((SIZE_LUT - 1) & bit_arr)];
        bit_arr = bit_arr >> CHAR_BIT;
    }
