...
int WDStart(char *argv[], int check_ratio, int beats_interval)
{
    ...
    InitHandler(IncramentCounterHandler, SIGNAL1);/*TODO: check falier!*/
    ...
    /* DEBUG_PRINT */printf(("WDsatrt, at the begining, is_WD: %d argv[0]: "));
    /* DEBUG_PRINT */printf(("%s argv[1]: %s n", 
                            watchdog_elem->is_WD, argv[0], argv[1]));
    sem_val =0;
