...
int WDStart(char *argv[], int check_ratio, int beats_interval)
{
    ...
    SetEnvVar(watchdog_elem, argv, check_ratio, beats_interval, SIGNAL, BLOCK);
...
static int SetEnvVar(watchdog_t *watchdog, char *argv[], int check_ratio, int beats_interval, char *sem_signa_name, char *sem_block_name)
{
    ...
    setenv(SEM_BLOCK_NAME, sem_block_name, 0);
    setenv(SEM_SIGNAL_NAME, sem_signa_name, 0);

    setenv(WD_APP, "/home/shelly/git/ds/bin/WD_file_for_user", 0);
