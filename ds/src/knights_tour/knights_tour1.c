...
static char GetNextPossibleStep(char curr_position, bit_arr_t chess_board, size_t index);
static return_val_t JumpToNextPosition(char curr_position, bit_arr_t chess_board, char path[BOARD_SIZE], clock_t start_time);

/*------------- Warnsdorffs decleration ------------*/
static return_val_t MoveToNextPosition(char curr_position, bit_arr_t chess_board, char path[BOARD_SIZE], clock_t start_time);
int CmpFunction(const void *data1, const void *data2);

static size_t GetDegree(char next_steps_lut[BOARD_AXIS], /* char curr_position, */ bit_arr_t chess_board);

static void GetPossibleSteps(next_step_t next_steps_lut[], char curr_position, bit_arr_t chess_board);
...
static char GetNextPossibleStep(char curr_position, bit_arr_t chess_board, size_t index)
{
    ...
    if (-1 != lut[(size_t)curr_position][index] && 0 == BitArrGetVal(chess_board, (size_t)lut[(size_t)curr_position][index]))
    {
...
static return_val_t MoveToNextPosition(char curr_position, bit_arr_t chess_board, char path[BOARD_SIZE], clock_t start_time)
{
    ...
    for (i = 0; (i < BOARD_AXIS) && (res != SUCCESS) && (res != TIME_OUT); ++i)
    {
        ...
        degree = (long int)next_steps_lut[i].degree;
...
{
    size_t i = 0;
    static char lut_warnsdorffs[BOARD_AXIS * BOARD_AXIS][BOARD_AXIS] =
        {{10, 17, -1, -1, -1, -1, -1, -1},
         {11, 18, 16, -1, -1, -1, -1, -1},
         {12, 19, 17, 8, -1, -1, -1, -1},
         {13, 20, 18, 9, -1, -1, -1, -1},
    ...
    memset(next_steps_lut, (char)0, BOARD_AXIS * sizeof(next_step_t));

    for (i = 0; (i < BOARD_AXIS); ++i)
    {
        if (-1 < lut_warnsdorffs[(size_t)curr_position][i])
        {
            next_steps_lut[i].next_step = lut_warnsdorffs[(size_t)curr_position][i];
            next_steps_lut[i].degree = GetDegree(lut_warnsdorffs[(size_t)next_steps_lut[i].next_step], chess_board);
        }
    }
...
static size_t GetDegree(char next_steps_lut[BOARD_AXIS], bit_arr_t chess_board)
{
    ...
    while (i < BOARD_AXIS)
    {
        if ((-1 != next_steps_lut[i]) && (0 == BitArrGetVal(chess_board, (size_t)next_steps_lut[i])))
        {
            ++counter;
        }
        ++i;
