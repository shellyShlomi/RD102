...
static char *GetLutData(char curr_position);
...
static char GetNextPossibleStep(char curr_position, bit_arr_t chess_board, size_t index)
{
    ...
    value = GetLutData(curr_position)[index];
...
static void GetPossibleSteps(next_step_t next_steps_lut[],
                             ...
                             bit_arr_t chess_board)
{
    char *next_steps = NULL;
    size_t i = 0;

    assert(curr_position >= 0);
    assert(curr_position < BOARD_SIZE);

    next_steps = GetLutData(curr_position);

    for (i = 0; (i < BOARD_AXIS); ++i)
    {
        if (-1 < next_steps[i])
        {
            next_steps_lut[i].next_step = next_steps[i];
            next_steps_lut[i].degree = GetDegree(GetLutData(next_steps_lut[i].next_step), chess_board);
        }
    }
...
static char *GetLutData(char curr_position)
{
    ...
    return (lut[(size_t)curr_position]);
}
